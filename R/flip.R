...
# We will only use this in our main code



create_blfdr_parts= function(u, param, extraParam,  X, gen_size){
  linear_predictors = cbind(1, X)%*%matrix(param, nr = 1 + ncol(X))
...
create_plfdr_fn = function(prob_pair, para,  null_col){
  plfdr_fn = function(z){
    num = prob_pair[null_col]*dnorm(z, mean = para[1, null_col], sd= para[2, null_col])
    denom = num + prob_pair[-null_col]*dnorm(z, mean = para[1, -null_col], sd= para[2, -null_col])
    return(num/denom)
...
flip = function(stat_vec , shell_fn_ls, norm_gen_size){
  ...
  norm.dat= rnorm(n = norm_gen_size)
  # use mclapply
  dat_ls <- mclapply(shell_fn_ls, FUN = function(f) {f(norm.dat)}, mc.cores = 8)

  obtain_flip = function(datvec, stat){
    quantile(datvec, 1 - sum(datvec < stat)/norm_gen_size )
  }
...
create_beta_lfdr_parts = function(z, param, extraParam,  X, gen_size){
  ...
  create_beta_lfdr_fn= function(param, extraParam){
    ...
    beta_lfdr_fn = function(z){
      beta_lfdr =
        (dbeta(pnorm(z), 1/(1 +exp(- param[1])), extraParam[1])*exp(param[3]) +
           dbeta(pnorm(z), extraParam[2],  1/(1 + exp(-param[2])))*exp(param[4])   +
           1)^(-1)
      return(beta_lfdr)
    }
    return(beta_lfdr_fn)
  ...
  linear_predictors = cbind(1, X)%*%matrix(param, nc = 4)
